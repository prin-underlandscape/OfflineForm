class Format {
  ...
  factory(fileName) {
    ...
    var inputNode=document.body.appendChild(el);
    var f = inputNode.addEventListener('change', (e) => {
      e.target.files[0].json().then((obj) => { this.description = obj });
	...
	format(type) {
		return this.description.find(x => x.formname === type);
	}	
	types() {
		return this.description.map(f => f.formname);
	}
}
